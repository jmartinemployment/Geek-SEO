...
public sealed class SiteAnalysisPersistenceTests
{
    [Fact]
    public void ScanFingerprint_IsStableForSameInputs()
    {
        ...
        var a = SiteScanFingerprint.Compute("example.com", "sul-2.0", schema, sitemap, nav);
        var b = SiteScanFingerprint.Compute("example.com", "sul-2.0", schema, sitemap, nav);
    ...
    public void TopicCandidateMapper_MapsSelectedAndExcluded()
    {
        ...
        var rows = SiteAnalysisTopicCandidateMapper.FromSiteTopicProfile(profileId, fused, includeEvidence: true);

        Assert.Equal(2, rows.Count);
        Assert.True(rows[0].IsSelected);
        Assert.False(rows[1].IsSelected);
        Assert.Equal("low confidence", rows[1].ExclusionReason);
        Assert.Equal(profileId, rows[0].SiteAnalysisProfileId);
    }

    [Fact]
    public void CompetitorBulkInsertMapper_PreservesScope()
    {
        var dto = new SiteAnalysisCompetitorBulkInsert(
            Guid.NewGuid(),
            Guid.NewGuid(),
            "delray-it.com",
            2,
        ...
        var entity = SiteAnalysisBulkInsertMapper.ToEntity(dto);
