...
public sealed class SiteExtractionTests
{
    ...
    public void PillarSelector_SelectsTwelveGeekAtYourSpotSchemaTopics()
    {
        ...
        Assert.Contains(
            result.Selected,
            p => p.Slug.Equals(SiteAnalyzerService.NameToSlug("AI Consulting"), StringComparison.OrdinalIgnoreCase));
        Assert.Contains(
            result.Selected,
            p => p.Slug.Equals(SiteAnalyzerService.NameToSlug("AI Strategy Consulting"), StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void PillarSelector_IncludesAccounting_WithAllSchemaTopics()
    {
        ...
        {
            Name = "Accounting",
            Slug = SiteAnalyzerService.NameToSlug("Accounting"),
        ...
        Assert.Contains(
            result.Selected,
            p => p.Slug.Equals(SiteAnalyzerService.NameToSlug("Accounting"), StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void GscQueryExtractor_ScorePillarMatch_MatchesAccountingQuery()
    {
        var score = GscQueryExtractor.ScorePillarMatch(
            "Accounting",
            SiteAnalyzerService.NameToSlug("Accounting"),
    ...
    public void GscQueryExtractor_ApplyToPool_AddsGscEvidence()
    {
        ...
        var accounting = updated.Single(c => c.Slug == SiteAnalyzerService.NameToSlug("Accounting"));
    ...
    public void SerpEntityExtractor_ExtractsUrlAndRelatedTopicSlugs()
    {
        ...
        Assert.Contains(SiteAnalyzerService.NameToSlug("bookkeeping for small business"), slugs);
        Assert.Contains(SiteAnalyzerService.NameToSlug("What is payroll outsourcing?"), slugs);
    }
    ...
    public void TopicCandidatePoolBuilder_StacksInternalLinkAndUrlPatternEvidence()
    {
        ...
        var accountingServices = pool.First(c =>
            c.Slug.Equals(SiteAnalyzerService.NameToSlug("Accounting Services"), StringComparison.OrdinalIgnoreCase));
    ...
    public void PillarSelector_ExcludesSingleSourceHeadingBelowMinConfidence()
    {
        var pool = new List<TopicCandidate>
        {
            new()
            {
                ...
                Slug = SiteAnalyzerService.NameToSlug("Random Body Phrase"),
        ...
        Assert.Contains(SiteAnalyzerService.NameToSlug("Accounting"), selectedSlugs);
        Assert.DoesNotContain(SiteAnalyzerService.NameToSlug("Random Body Phrase"), selectedSlugs);
        // sul-2.0: heading-only (0.10) is below MinPillarConfidence (0.15); page_vertical passes.
        Assert.True(fused.ExclusionReasons.ContainsKey(SiteAnalyzerService.NameToSlug("Random Body Phrase")));
    }
    ...
    [Fact]
    public void SiteContentCoverageMatcher_MarksDedicatedPageAsCovered()
    {
        var profileId = Guid.NewGuid();
        var pillarId = Guid.NewGuid();
        var pillars = new List<SiteAnalysisPillar>
        {
            new()
            {
                Id = pillarId,
        ...
        var result = SiteContentCoverageMatcher.Apply(
            pillars,
            subtopics,
            fused,
            discovered,
    ...
    [Fact]
    public void SiteContentCoverageMatcher_SchemaOnlyPillarIsPartialOrGap()
    {
        var pillarId = Guid.NewGuid();
        var pillars = new List<SiteAnalysisPillar>
        {
    ...
    [Fact]
    public void SiteTopicalMapSeedResolver_PrefersActionAndGapPillars()
    {
        var fused = new SiteTopicProfile
        {
            AllCandidates = [],
    ...
    [Fact]
    public void SiteTopicalMapSeedResolver_MatchPillarName_FindsSlugOverlap()
    {
        var pillars = new[]
        {
            new TopicCandidate
        ...
        var match = SiteTopicalMapSeedResolver.MatchPillarName("best accounting software tools", pillars);
