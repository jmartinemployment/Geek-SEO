#!/usr/bin/env node
/**
 * Production site-analyzer API check — latest profile must include pillars when complete.
...
try {
  ...
  if (latest.status === 204) {
    console.log('○ no site analysis profile yet (204)');
    process.exit(0);
  }
  ...
  console.log('\nAll site-analyzer production checks passed.');
} catch (error) {
