...
export function PricingCheckout({ tiers }: Readonly<{ tiers: TierDefinition[] }>) {
  ...
  useEffect(() => {
    if (!sdkReady || !plans?.configured || !plans.clientId || !plans.planIds || !billingUserId) return;
    if (!window.paypal || renderedRef.current) return;

    renderedRef.current = true;

    for (const tier of tiers) {
      const planId = plans.planIds[tier.key];
  ...
  }, [billingUserId, plans, refreshSubscription, sdkReady, tiers]);
  ...
  const billingConfigured = plans?.configured === true;
  const activePaid =
  ...
  return (
    <>
      {billingConfigured && plans?.clientId ? (
        <Script
          src={`https://www.paypal.com/sdk/js?client-id=${encodeURIComponent(plans.clientId)}&vault=true&intent=subscription`}
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {tiers.map((tier) => (
          ...
          >
            ...
            {billingConfigured && isAuthenticated && billingUserId ? (
              ...
              >
                {billingConfigured ? 'Sign in to subscribe' : 'Billing not configured yet'}
              </button>
            )}
          </article>
        ))}
