  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-10">
      ...
      <ul className="mt-10 space-y-4">
          ...
          <li key={v.id} className="rounded-xl border bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 line-clamp-3 text-sm text-[var(--color-text-secondary)]">{v.sampleText}</p>
                {v.styleInstructions ? (
                  <p className="mt-2 text-xs text-[var(--color-text-secondary)]">{v.styleInstructions}</p>
                ) : null}
              </div>
              <button
                type="button"
        ...
        {voices.length === 0 ? (
          <p className="text-sm text-[var(--color-text-secondary)]">No brand voices yet.</p>
        ) : null}
      </ul>
    </main>
  );
