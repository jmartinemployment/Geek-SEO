...
**Implemented, deployed (unit-tested, not yet live-verified against a real domain):** sitemap **generate/update as step 1** on every Analyze — uncapped same-origin discovery, persist discovered URLs (`SourceType` `sitemap`|`generated`), auto-update `sitemap.xml` artifact + Download UI, then **inventory-complete** crawl (no `MaxSiteCrawlPages` / attempt-budget soft-stop). Utility pages are crawled for inventory; heading filtering is now "if heading its valid" — `NoisePaths`/`H2Noise`/`IsDuplicateHeading`/`Length` filters deleted (h1–h6, including h5, duplicates kept; `HeadingPillarBuilder` kept per product direction, `PageSectionTreeBuilder` = real h1–h6 tree). Crawl is render-always (Playwright); robots.txt is obeyed; there is no HTTP-HTML fallback.
...
**Fixed:** content-gap detection previously fabricated gaps — a pillar with <3 real crawled child pages was padded with 5 hardcoded generic subtopic templates instead of finding real ones. Now uses real heading-based gap detection: `RunSiteCrawlAsync` extracts real H1-H6 per crawled page, persisted into `SiteAnalysisProfileHeading`; `SiteContentCoverageMatcher.HasNoMatchingPage` flags headings with no matching crawled/sitemap URL as gaps. Shipped alongside a broader elimination of silent fallback/soft-success patterns (provider auto-switching, swallowed exceptions, artifact-blob substitution) across this repo and GeekAPI — see `/Users/jeffmartin/development/GeekContentCreator/docs/FALLBACK_ELIMINATION_PLAN.md` (inventory at `docs/FALLBACK_INVENTORY.md` in that repo). 195/195 tests pass; not yet live-verified against a real domain.
