...
public sealed class SiteAnalyzerController(
    ...
    ILogger<SiteAnalyzerController> logger) : ControllerBase
{
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            if (!SiteAnalyzerService.IsWorkerConfigured())
    ...
    public async Task<IActionResult> GetAnalysisDetails(Guid profileId, CancellationToken ct)
    {
        try
        {
            ...
            if (result.Value is null)
                return Ok(new SiteAnalysisDetails(1, [], null, SiteAnalysisStepCatalog.ToDtos()));

            var row = result.Value;
            if (!SiteAnalysisDetailsPolicy.IsStepLogAvailable(row.Status))
                return Ok(new SiteAnalysisDetails(row.AnalysisStepLogVersion, [], null, SiteAnalysisStepCatalog.ToDtos()));

            var steps = SiteAnalysisStepLogJson.Parse(row.AnalysisStepLog);
            ...
            if (row.Status is "complete")
            {
                ...
                {
                    fusion = await SiteAnalyzerStepRunState.LoadMergedFusionSnapshotAsync(
                        profileRepo,
                        profileId,
            ...
            return Ok(new SiteAnalysisDetails(row.AnalysisStepLogVersion, steps, fusion, SiteAnalysisStepCatalog.ToDtos()));
        }
        catch (InvalidOperationException ex)
        {
            return BadRequest(new { error = ex.Message });
    ...
    public async Task<IActionResult> GetStatus(Guid profileId, CancellationToken ct)
    {
        try
        {
            ...
            return Ok(new SiteAnalysisStatus(
                p.Id,
                p.Status,
                step,
                stepNumber,
        ...
        {
            logger.LogWarning(ex, "Transient error fetching site analysis status for profile {ProfileId}", profileId);
            return StatusCode(503, new { error = "Status temporarily unavailable" });
        }
    }
    ...
    public async Task<IActionResult> GetProfile(Guid profileId, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(ex, "Transient error fetching site analysis profile {ProfileId}", profileId);
    ...
    public async Task<IActionResult> GetCoverageMatrix(Guid profileId, CancellationToken ct)
    {
        try
        {
            ...
            return Ok(SiteAnalysisRelationalAnalyticsBuilder.BuildCoverageMatrix(profile.Value));
        }
        catch (InvalidOperationException ex)
        {
            return BadRequest(new { error = ex.Message });
        ...
        Guid profileId, [FromQuery] bool quickWinsOnly = false, CancellationToken ct = default)
    {
        try
        {
            ...
            return Ok(SiteAnalysisRelationalAnalyticsBuilder.BuildTopicalGaps(profile.Value, quickWinsOnly));
        }
        catch (InvalidOperationException ex)
        {
            return BadRequest(new { error = ex.Message });
        ...
        try
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    var stepRerunService = scope.ServiceProvider.GetRequiredService<SiteAnalyzerStepRerunService>();
                ...
                catch (Exception ex)
                {
                    ...
                    try
                    {
                        ...
                        var failureRecorder = errorScope.ServiceProvider.GetRequiredService<SiteAnalyzerStepRerunService>();
                        SiteAnalysisStepCatalog.BySlug.TryGetValue(slug, out var definition);
    ...
    public async Task<IActionResult> GetHistory(Guid projectId, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(ex, "Transient error fetching site analysis history for project {ProjectId}", projectId);
        ...
        Guid projectId, [FromQuery] int months = 12, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                logger.LogWarning(
                    "Site authority progress unavailable for project {ProjectId}: {Error}",
                    projectId,
                    result.Error);
            }
        ...
        {
            logger.LogWarning(ex, "Site authority progress failed for project {ProjectId}", projectId);
            return Ok(Array.Empty<AuthorityProgressPoint>());
        }
    }
    ...
    public async Task<IActionResult> GetLatest(Guid projectId, CancellationToken ct)
    {
        try
        {
            ...
            {
                logger.LogWarning(
                    "Failed to fetch latest site analysis profile for project {ProjectId}: {Error}",
                    projectId,
                    result.Error);
        ...
        {
            logger.LogWarning(ex, "Transient error fetching latest site analysis profile for project {ProjectId}", projectId);
            return StatusCode(503, new { error = "Latest profile temporarily unavailable" });
        }
    }
    ...
    private static SiteCompetitorResult MapCompetitor(SiteAnalysisCompetitor c) => new(
    ...
    private static SiteAnalysisProfileResult MapToResult(SiteAnalysisProfile p) => new()
    {
        Id = p.Id,
        ...
        Pillars = p.Pillars.OrderBy(x => x.DisplayOrder).Select(pi => new SiteAnalysisPillarResult
        {
            Id = pi.Id,
            ...
            Subtopics = pi.Subtopics.Select(s => new SiteAnalysisSubtopicResult
            {
                Id = s.Id,
        ...
        Entities = p.Entities.Select(e => new SiteAnalysisEntityResult(
