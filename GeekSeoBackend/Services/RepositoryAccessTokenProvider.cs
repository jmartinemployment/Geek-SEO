...
/// <summary>
/// Machine token for GeekSeoBackend → GeekRepository (client <c>geekseo-backend</c>, not <c>geekapi</c>).
/// </summary>
public sealed class RepositoryAccessTokenProvider
{
    ...
    public RepositoryAccessTokenProvider(IHttpClientFactory httpClientFactory)
    {
        ...
        _clientId = GeekOAuthConstants.GeekSeoBackendClientId;
        _clientSecret = Environment.GetEnvironmentVariable("GEEK_SEO_BACKEND_CLIENT_SECRET") ?? string.Empty;
        var issuer = Environment.GetEnvironmentVariable("GEEK_OAUTH_AUTHORITY")
    ...
    {
        if (string.IsNullOrWhiteSpace(_clientSecret) || string.IsNullOrWhiteSpace(_tokenEndpoint))
            return null;
