...
/// </summary>
public sealed class SiteAnalyzerStepRerunService(
    ISiteAnalysisProfileRepository profileRepo,
    ISiteAnalysisAnalyticsRepository analyticsRepo,
    ...
    ILogger<SiteAnalyzerStepRerunService> logger)
{
    ...
    {
        if (!SiteAnalysisStepCatalog.BySlug.TryGetValue(slug, out var definition))
        ...
        try
        {
            ...
            var downstream = SiteAnalysisStepCatalog.GetDownstream(slug);
            ...
            var slimEntry = SiteAnalyzerStepArtifactStore.ForStepLogPersistence(entry);
    ...
    public async Task RecordStepFailureAsync(
        ...
        Guid userId,
        string slug,
        SiteAnalysisStepDefinition? definition,
        string message,
    ...
    private async Task<SiteAnalysisStepLogEntry> ExecuteStepAsync(
        Guid profileId, Guid userId, string slug, IBrowser? browser, CancellationToken ct)
    {
        var profileResult = await profileRepo.GetByIdAsync(profileId, ct);
        if (!profileResult.IsSuccess || profileResult.Value is null)
    ...
    private async Task<SiteAnalysisStepLogEntry> RerunSchemaAsync(
        Guid profileId, string domain, IBrowser? browser, CancellationToken ct)
    {
        ...
        return SiteAnalysisStepLogBuilder.Schema(1, data, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunSiteUrlsAsync(
        Guid profileId, string domain, CancellationToken ct)
    {
        ...
        var msg = data.TotalUrlsScanned > 0
            ...
            : "No URLs in sitemap.";
        return SiteAnalysisStepLogBuilder.SiteUrls(2, data, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunNavAsync(
        Guid profileId, string domain, IBrowser? browser, CancellationToken ct)
    {
        ...
        return SiteAnalysisStepLogBuilder.Nav(3, data, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunHeadingsAsync(
        Guid profileId, string domain, IBrowser? browser, CancellationToken ct)
    {
        ...
        return SiteAnalysisStepLogBuilder.Headings(4, data, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunPageContentAsync(
        Guid profileId, string domain, IBrowser? browser, CancellationToken ct)
    {
        ...
        var persistContent = await profileRepo.ReplacePageContentAsync(
            profileId,
            SiteAnalyzerStepRelationalLoader.ToPageContentWrite(domain, data),
            ct);
        ...
        return SiteAnalysisStepLogBuilder.PageContent(5, data, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunSiteStructureAsync(
        Guid profileId, string domain, IBrowser? browser, CancellationToken ct)
    {
        ...
        return SiteAnalysisStepLogBuilder.SiteStructure(6, crawlData, internalLinks, urlPatterns, msg);
    }
        ...
        Guid profileId, Guid userId, SiteAnalysisProfile profile, string domain, IBrowser? browser, CancellationToken ct)
    {
        ...
        return SiteAnalysisStepLogBuilder.Merging(7, pool.Count, merged.Count, merged,
    ...
    private async Task<SiteAnalysisStepLogEntry> RerunKeywordsAsync(
        Guid profileId, SiteAnalysisProfile profile, string domain, CancellationToken ct)
    {
        ...
        return SiteAnalysisStepLogBuilder.Keywords(8, demand, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunSerpValidationAsync(
        Guid profileId, SiteAnalysisProfile profile, string domain, CancellationToken ct)
    {
        var fused = await LoadFusionAsync(profileId);
        var merged = fused.SelectedPillars.Select(PillarSelector.ToDiscoveredPillar).ToList();
        ...
        var headings = await SiteAnalyzerStepRelationalLoader.LoadHeadingsAsync(profileRepo, profileId, domain, steps, ct);
        var demand = await pillarDemandEnricher.EnrichAsync(
        ...
        return SiteAnalysisStepLogBuilder.SerpValidation(9, demand, msg);
    }

    // ── Steps 10–14: Synthesis steps — read from stored data ──────────────

    private async Task<SiteAnalysisStepLogEntry> RerunProfileAsync(
        Guid profileId, string domain, CancellationToken ct)
    {
        var fused = await LoadFusionAsync(profileId);
        var merged = fused.SelectedPillars.Select(PillarSelector.ToDiscoveredPillar).ToList();
        var profileResult = await profileRepo.GetByIdAsync(profileId, ct);
        var siteAnalysisProfile = profileResult.Value!;
        var schemaData = await schemaExtractor.ExtractAsync(domain, null, ct);
        var pillars = BuildSiteAnalysisPillarsLight(merged, profileId);
        var rootEntity = rootBuilder.Build(schemaData, new HomepageHeadings(), pillars);
        var audience = "local_service";
        var tags = BuildFocusTagsLight(schemaData, pillars);
        var msg = $"site analysis profile: {rootEntity}.";
        return SiteAnalysisStepLogBuilder.Profile(10, rootEntity, audience, tags, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunLocalAsync(
        Guid profileId, string domain, CancellationToken ct)
    {
        var fused = await LoadFusionAsync(profileId);
        var merged = fused.SelectedPillars.Select(PillarSelector.ToDiscoveredPillar).ToList();
        ...
        return localGeo.IsLocalBusiness
            ? SiteAnalysisStepLogBuilder.Local(11, localGeo, msg)
            : SiteAnalysisStepLogBuilder.LocalDisabled(11, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunCoverageAsync(
        Guid profileId, string domain, CancellationToken ct)
    {
        var fused = await LoadFusionAsync(profileId);
        var crawlUrls = await LoadCrawledUrlsAsync(profileId, ct);
        ...
        var coverage = SiteContentCoverageMatcher.Apply(
        ...
        return SiteAnalysisStepLogBuilder.Coverage(12,
    ...
    private async Task<SiteAnalysisStepLogEntry> RerunScoringAsync(Guid profileId, CancellationToken ct)
    {
        var fused = await LoadFusionAsync(profileId);
        var profileFull = await profileRepo.GetByIdAsync(profileId, ct);
        var pillars = profileFull.Value!.Pillars.ToList();
        ...
        await persistence.SaveCompletionAsync(profileId,
            new SiteAnalysisProfileSummaryPatch(
                profileFull.Value.PrimaryFocus, profileFull.Value.FocusDescription,
                profileFull.Value.FocusTags, profileFull.Value.AudienceType,
        ...
        return SiteAnalysisStepLogBuilder.Scoring(13, score, covered, partial, gap,
            pillars.Count, msg);
    }

    private async Task<SiteAnalysisStepLogEntry> RerunCompleteAsync(Guid profileId, CancellationToken ct)
    {
        var now = DateTimeOffset.UtcNow;
        await profileRepo.UpdatePhaseStatusAsync(profileId,
            new SiteAnalysisPhaseStatusPatch(StructureStatus: "complete", EnrichmentStatus: "complete",
                PersistStage: "done"), ct);
        await profileRepo.UpdateStatusAsync(profileId, "complete", "complete", 14, 14, ct: ct);
        return SiteAnalysisStepLogBuilder.Complete(14, now, now.AddDays(30), "Analysis complete!");
    }
    ...
    private static List<SiteAnalysisPillar> BuildSiteAnalysisPillarsLight(
        IReadOnlyList<DiscoveredPillar> merged, Guid profileId) =>
        merged.Select((p, i) => new SiteAnalysisPillar
        {
    ...
    private static string[] BuildFocusTagsLight(SchemaOrgData schema, List<SiteAnalysisPillar> pillars) =>
        ...
        string profileStatusBeforeRun)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            try
            {
                ...
                var repo = scope.ServiceProvider.GetRequiredService<ISiteAnalysisProfileRepository>();
                var persistLogger = scope.ServiceProvider.GetRequiredService<ILogger<SiteAnalyzerStepRerunService>>();
