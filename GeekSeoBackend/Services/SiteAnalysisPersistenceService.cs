...
/// <summary>
/// Orchestrates split PATCH writes for site analysis completion (Phase 1+).
/// Falls back to monolithic <see cref="ISiteAnalysisProfileRepository.SaveAnalysisResultsAsync"/> when split routes are unavailable.
/// </summary>
public sealed class SiteAnalysisPersistenceService(
    ISiteAnalysisProfileRepository profileRepo,
    ILogger<SiteAnalysisPersistenceService> logger)
{
    ...
    {
        var rows = GeekSeo.Application.Mapping.SiteAnalysisTopicCandidateMapper
        ...
        if (includeEvidence)
        {
            ...
            var evidenceRows = fused.AllCandidates
                ...
                .SelectMany(c => c.Evidence.Select((e, index) => new SiteAnalysisTopicCandidateEvidenceWrite(
        ...
        await profileRepo.UpdatePhaseStatusAsync(
            profileId,
            new SiteAnalysisPhaseStatusPatch(null, null, PersistStage: "candidates"),
            ct);
        return Result.Success();
    }

    private async Task<Result<IReadOnlyList<SiteAnalysisTopicCandidatePage>>> LoadAllPersistedCandidatesAsync(
        Guid profileId,
        CancellationToken ct)
    {
        ...
        while (true)
        {
            var result = await profileRepo.GetTopicCandidatesAsync(
                ...
                ct);
            if (!result.IsSuccess)
                return Result<IReadOnlyList<SiteAnalysisTopicCandidatePage>>.Failure(result.Error ?? "Failed loading topic candidates.");
            if (result.Value is null)
                return Result<IReadOnlyList<SiteAnalysisTopicCandidatePage>>.Failure("Topic candidate response was empty.");
        ...
        return Result<IReadOnlyList<SiteAnalysisTopicCandidatePage>>.Success(items);
    }
